  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      ...
      {tenants.isError && !data ? (
        ...
        // Virtualized: the tenant roster grows with every café onboarded.
        <FlashList
          data={tenants.isLoading ? [] : (data?.tenants ?? [])}
          ...
          renderItem={({ item: t }) => <TenantRow tenant={t} onOpen={setDetail} />}
        />
      )}
...
/** One tenant row in the platform console. Memoized for long rosters. */
const TenantRow = memo(function TenantRow({
  tenant: t,
  onOpen,
}: {
  tenant: AdminTenant;
  onOpen: (t: AdminTenant) => void;
}) {
  const theme = useTheme();
  return (
